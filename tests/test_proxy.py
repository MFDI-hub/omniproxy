...
class TestProxy(unittest.TestCase):
    def test_input_formats(self):
        ...
        self.assertEqual(
            Proxy('https://login:password@210.173.88.77:3001').__str__(),
            'http://login:password@210.173.88.77:3001',
        )
        self.assertEqual(
            Proxy('socks5://login:password@210.173.88.77:3001').__str__(),
            'socks5://login:password@210.173.88.77:3001',
        )
        self.assertEqual(
            Proxy('socks4://login:password@210.173.88.77:3001').__str__(),
            'socks4://login:password@210.173.88.77:3001',
        )
        self.assertEqual(Proxy('socks5://210.173.88.77:3001').__str__(), 'socks5://210.173.88.77:3001')
        self.assertEqual(Proxy('socks5://myproxy.com:3001').__str__(), 'socks5://myproxy.com:3001')
    ...
    @patch("httpx.request")
    def test_rotate(self, mock_sync_request):
        mock_sync_request.return_value = MagicMock(status_code=200)
        inner = MagicMock()
        inner.request = AsyncMock(return_value=MagicMock(status_code=200))
        cm = MagicMock()
